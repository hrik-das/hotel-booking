<?php
    ...
    function sendMail($useremail, $token, $type) {
        ...
        $email->addContent(
            ...
            <a href='".SITE_URL."$page?$type&email=$useremail&token=$token"."'>Confirm</a>"
        );
    ...
    if (isset($_POST["register"])) {
        ...
        $userExist = executeCrud("select", "SELECT * FROM `user_cred` WHERE `email`=? OR `phone`=? LIMIT 1", [$data["email"], $data["phone"]], "ss");
        ...
        if ($image == "invalidImage") {
            ...
            if (executeCrud("insert", $query, $values, "sssssssss")) {
                echo 1;
            } else {
                echo "insertFailed";
            }
    ...
    if (isset($_POST["login"])) {
        $data = filteration($_POST);
        $userExist = executeCrud("select", "SELECT * FROM `user_cred` WHERE `email`=? OR `phone`=? LIMIT 1", [$data["email_mob"], $data["email_mob"]], "ss");
        if (mysqli_num_rows($userExist) == 0) {
            echo "invalidEmailorPhone";
            exit();
        } else {
            $userFetch = mysqli_fetch_assoc($userExist);
            if ($userFetch["is_verified"] == 0) {
                echo "notVerified";
                ...
                if (!(password_verify($data["pass"], $userFetch["password"]))) {
                    ...
                    $_SESSION["userimage"] = $userFetch["profile"];
    ...
    if (isset($_POST["forgot"])) {
        $data = filteration($_POST);
        $userExist = executeCrud("select", "SELECT * FROM `user_cred` WHERE `email`=? LIMIT 1", [$data["email"]], "s");
        if (mysqli_num_rows($userExist) == 0) {
            echo "invalidEmail";
        } else {
            $userFetch = mysqli_fetch_assoc($userExist);
            if ($userFetch["is_verified"] == 0) {
                ...
                if (!(sendMail($data["email"], $token, "account_recovery"))) {
                    ...
                    $date = date("Y-m-d");
                    $query = "UPDATE `user_cred` SET `token`='$token', `token_expire`='$date' WHERE `id`='$userFetch[id]'";
    ...
    if (isset($_POST["recover"])) {
        ...
        if (executeCrud("update", $query, $values, "sssss")) {
            echo 1;
        } else {
            echo "failed";
        }
