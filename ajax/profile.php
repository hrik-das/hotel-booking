<?php
    ...
    if (isset($_POST["info-form"])) {
        ...
        session_start();
        $userExist = executeCrud("select", "SELECT * FROM `user_cred` WHERE `phone`=? AND `id`!=? LIMIT 1", [$data["phone"], $_SESSION["userid"]], "ss");
        ...
        if (executeCrud("update", $query, $values, "sssssi")) {
    ...
    if (isset($_POST["profile-form"])) {
        ...
        $userExist = executeCrud("select", "SELECT `profile` FROM `user_cred` WHERE `id`=? LIMIT 1", [$_SESSION["userid"]], "i");
        ...
        if (executeCrud("update", $query, $values, "ss")) {
    ...
    if (isset($_POST["pass-form"])) {
        ...
        if (executeCrud("update", $query, $values, "si")) {
            echo 1;
        } else {
            echo 0;
        }
