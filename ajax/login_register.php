<?php
    ...
    function sendMail($useremail, $username, $token, $type){
        ...
        $email->setFrom("hrikdas012@gmail.com", "Godlike Restaurant");
    ...
    if(isset($_POST["register"])){
        ...
        if($image == "invalidImage"){
            ...
            if(!(sendMail($data["email"], $data["name"], $token, "emailConfirmation"))){
                echo "mailFailed";
                exit();
            }
            $encryptPassword = password_hash($data["pass"], PASSWORD_BCRYPT);
            $query = "INSERT INTO `user_cred`(`name`, `email`, `address`, `phone`, `pincode`, `dob`, `profile`, `pass`, `token`) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
