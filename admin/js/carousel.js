...
let carouselPictureInput = document.getElementById("carousel-picture-input");
    ...
    xhr.onload = function() {
        if (xhr.status >= 200 && xhr.status < 300) {
            ...
            if (this.responseText == "invalidImage") {
                ...
                alert("success", "New Image Added for Carousel.");
    ...
    xhr.onload = function() {
        if (xhr.status >= 200 && xhr.status < 300) {
            if (this.responseText == 1) {
                alert("success", "Carousel Image Deleted.");
                getCarousel();
            } else {
                alert("error", "Cannot Delete Carousel Image!");
            }
        } else {
            console.error("Request failed with status : ", xhr.status);
        }
