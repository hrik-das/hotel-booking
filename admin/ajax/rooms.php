<?php
    ...
    if(isset($_POST["removeRoom"])){
        $filterData = filteration($_POST);
        $result1 = select("SELECT * FROM `room_image` WHERE `room_id`=?", [$filterData["room_id"]], "i");
        while($data = mysqli_fetch_assoc($result1)){
            deleteImage($data["image"], ROOM_FOLDER);
        }
        $result2 = delete("DELETE FROM `room_image` WHERE `room_id`=?", [$filterData["room_id"]], "i");
        $result3 = delete("DELETE FROM `room_features` WHERE `room_id`=?", [$filterData["room_id"]], "i");
        $result4 = delete("DELETE FROM `room_facilities` WHERE `room_id`=?", [$filterData["room_id"]], "i");
        $result5 = update("UPDATE `rooms` SET `removed`=? WHERE `id`=?", [1, $filterData["room_id"]], "ii");
