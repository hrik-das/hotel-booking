...
<body class="bg-light">
    ...
    <?php
        ...
        $roomResult = executeCrud("select", "SELECT * FROM `rooms` WHERE `id`=? AND `status`=? AND `removed`=?", [$data["id"], 1, 0], "iii");
                    ...
                    <div class="card-body">
                        <?php
                            ...
                            if (!($settings_r["shutdown"])) {
                                ...
                                echo<<<book
                                    <button onclick='checkLoginToBook($login, $data[id])' class='btn w-100 text-white custom-background shadow-none mb-1'><i class='bi bi-bookmark-fill'></i> Book Now</button>
                                book;
                            }
                        ?>
                    </div>
